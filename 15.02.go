...
func expandGrid(original Grid) Grid {
	ySize := len(original)
	xSize := len(original[0])
	expand := 5

	grid := make([][]int, ySize*expand)

	for y := 0; y < ySize*expand; y++ {
		grid[y] = make([]int, xSize*expand)
	}

	for yMult := 0; yMult < expand; yMult++ {
		for xMult := 0; xMult < expand; xMult++ {
			yBase := yMult * ySize
			for y := yBase; y < (yMult+1)*ySize; y++ {
				xBase := xMult * xSize
				for x := xBase; x < (xMult+1)*xSize; x++ {
					newVal := original[y-yBase][x-xBase] + xMult + yMult
					if newVal > 9 {
						newVal = newVal % 9
					}
					grid[y][x] = newVal
