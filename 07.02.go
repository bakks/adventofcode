...
func calculateFuelCost(crabs []int, target int, costSeq []int) int {
	cost := 0

	for _, crab := range crabs {
		cost += costSeq[abs(crab-target)]
	}

	return cost
}

func generateCostSequence(length int) []int {
	seq := []int{}
	acc := 0
...
func calculateBestTarget(crabs []int) int {
	min := minArr(crabs)
	max := maxArr(crabs)
	costSeq := generateCostSequence(max - min + 1)

	lowestCostTarget := -1
	lowestCost := -1

	for i := min; i <= max; i++ {
		cost := calculateFuelCost(crabs, i, costSeq)
